...
likeDebayR=(3./dens)^(1./3.)                % "Debay" sphere with 3 electrons, cm
...
debayR=zeros(nVion,1);        % Debay radius, cm
rhoMax=zeros(nVion,1);        % maximal impact parameter, cm
rhoMin=zeros(nVion,1);        % minimal impact parameter, cm
%    
% Number interactions between ion electron during its passing near ion:
%
N_l=zeros(nVion,1);           % low 
N_s=zeros(nVion,1);           % superlow 
...
CL_ms=log(.5*likeDebayR*rmsRoLarm.^(-1));                     % 3 values approx 2.
...
for n=1:nVion
    ...
    if (absVion > rmsLongVe) 
        ...
        debayR(n)=absVion/omega_p;
    else
        N_s(n)=1+round(rmsTrnsvVe/(pi*rmsLongVe));
        debayR(n)=rmsLongVe/omega_p;
    end
...
%
    for k=1:3
        ...
        num=min(absVion/omega_p,likeDebayR);
        CL_ml(n,k)=log(.5*num/rmsRoLarm(k));
        CL_fl(n,k)=CL_fh(n,k);
% 
% Friction forces:
%
        trnsvFF_sl(n,k)=ffForm*relVion(n)*(CL_ms(k)/relVion(n)^3) ...
                                           +2*relVeTrnsv^3*(CL_fs(k)+N_s(n)*CL_as);
        trnsvFF_l(n,k)=ffForm*relVion(n)*(CL_ml(n,k)/relVion(n)^3) ...
                                          +2*relVeTrnsv^3*(CL_fl(n,k)+N_l(n)*CL_al);
        trnsvFF_h(n,k)=ffForm*(2.*CL_fh(n,k)+CL_mh(n,k))/relVion(n)^2;
        longFF_sl(n,k)=ffForm*relVion(n)*(CL_ms(k)/relVion(n)^3) ...
                                          +2*relVeTrnsv^2*relVeLong*(CL_fs(k)+N_s(n)*CL_as);
        longFF_l(n,k)=ffForm*relVion(n)*(2./relVion(n)^3) ...
                                         +2*relVeTrnsv^2*(CL_fl(n,k)+N_l(n)*CL_al);
        longFF_h(n,k)=ffForm*(2.*CL_fh(n,k)+2.)/relVion(n)^2;
    end
end

figure(5)
loglog(relVion,debayR,'-r','LineWidth',2)
grid on
xlabel('Relative Ion Velocity, V_{i\perp}/V_{e0}','Color','m','FontSize',16)
ylabel('R_{Debay}, cm','Color','m','FontSize',16)
title('Debay Radius R_{Debay}','Color','m','FontSize',16)
ylim([8.e-4,.15])
...
figure(100)
loglog(relVion(1:48),trnsvFF_sl(1:48,1),'-r','LineWidth',2)
grid on
% xlabel('Relative Ion Velocity, V_{i\perp}/V_{e0}','FontSize',16,'Color','m')
% xlim([1.e-6,1./relVeLong])
ylabel('F_\perp, MeV/m','Color','m','FontSize',16)
title('Transverse Friction Force F_\perp','Color','m','FontSize',16)
   
figure(110)
loglog(relVion(49:90),trnsvFF_l(49:90,1),'-b','LineWidth',2)
grid on
% xlabel('Relative Ion Velocity, V_{i\perp}/V_{e0}','FontSize',16,'Color','m')
ylabel('F_\perp, MeV/m','Color','m','FontSize',16)
title('Transverse Friction Force F_\perp','Color','m','FontSize',16)
% xlim([1./relVeLong,1./relVeTrnsv])

figure(120)
loglog(relVion(91:100),trnsvFF_h(91:100,1),'-m','LineWidth',2)
grid on
% xlabel('Relative Ion Velocity, V_{i\perp}/V_{e0}','FontSize',16,'Color','m')
ylabel('F_\perp, MeV/m','Color','m','FontSize',16)
title('Transverse Friction Force F_\perp','Color','m','FontSize',16)
% xlim([1./relVeTrnsv,1.e-2])

fig7trnsv=zeros(nVion,3);
fig7long=zeros(nVion,3);

for n=1:48
    fig7trnsv(n,:)=trnsvFF_sl(n,:);
    fig7long(n,:)=longFF_sl(n,:);
end
...
figure(200)
loglog(relVion,fig7trnsv(:,1),'-r',relVion,fig7trnsv(:,2),'-b', ...
       relVion,fig7trnsv(:,3),'-m','LineWidth',2)
grid on
% xlabel('Relative Ion Velocity, V_{i\perp}/V_{e0}','FontSize',16,'Color','m')
% xlim([1.e-6,1./relVeLong])
ylabel('F_\perp, MeV/m','Color','m','FontSize',16)
