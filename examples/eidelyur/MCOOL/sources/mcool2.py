...
"""
Guiding-center Coordinates and $\Theta$-J Coordinates
Transformations To and From Guiding-center Coordinates and $\Theta$-J 
Coordinates

We transform the electron's transverse phase-space coordinates
using the type-1 generating function $$ F_1(x,y;\, 
\phi,\ygc) = m\Omega\Bigl[\frac{1}{2}(y - \ygc)^2\cot\phi - y \ygc\Bigr]. 
$$ This yields the following transformation rules:
to guiding-center coordinates $$ \begin{align} m\Omega &amp;= qB_0, 
\quad\text{(this is a signed quantity)}\\[1ex] 
\phi &amp;= \arctan\Bigl(\frac{p_x + e B y}{p_y}\Bigr),\\[1ex] 
p_\phi &amp;= \frac{1}{2m\Omega}\bigl[(p_x + m\Omega y)^2 + p_y^2\bigr],
\\[1ex] \ygc &amp;= -\frac{p_x}{m\Omega},\\[1ex] 
\pgc &amp;= p_y + m\Omega x. \end{align} $$ from guiding-center 
coordinates $$ \begin{align} 
r_L &amp;= \frac{1}{m\Omega}\sqrt{2m\Omega\,p_\phi}, 
\quad\text{(this is a signed quantity)}\\[1ex] 
x &amp;= \frac{\pgc}{m\Omega} - r_L\cos\phi,\\[1ex] 
p_x &amp;= -m\Omega\,\ygc,\\[1ex] 
y &amp;= \ygc + r_L\sin\phi,\\[1ex] p_y &amp;= m\Omega\,r_L\cos\phi. 
\end{align} $$

We also require the transformation to and from the coordinates 
$\Theta$-J: $$ \begin{align} \Theta &= \dotsb, \ 
J &= p_\phi + \frac{Ze^2}{4\pi\varepsilon_0} 
\frac{r_L}{\Omega} \frac{(\Qion{x}-\xgc)\cos\phi - 
(\Qion{y}-\ygc)\sin\phi}{% \bigl[(\Qion{x}-\Qe{x})^2 + 
(\Qion{y}-\Qe{y})^2 + (\Qion{z}-\Qe{z})^2
...
print "b / micro =    ", impactParameterB / micro
